...
Floor::Floor(std::shared_ptr<Player> pc, int currFloor) : player(pc), currFloor(currFloor) {
    Generator::generate();
    shared_ptr<TextDisplay> td = make_shared<TextDisplay>(player, currFloor);
}
...
void Floor::readLayout(istream &in){
    ...
    for (auto objType : content) {
        if ((objType == ObjectType::Wall_v || objType == ObjectType::Wall_h) && !this->searchInChambers(coordRow, coordCol){
            ...
            void Floor::notify(std::shared_ptr<Subject> s){
                ...
                if ((s->getType() != SubjectType::Potion) && (s->getType() != SubjectType::Gold)){
                    shared_ptr<Character> character = dynamic_pointer_cast<Subject> (s);
                    this->gold = character->getGold();
                    this->hp = character->getHP();
                    this->atk = character->getAttack();
                    this->def = character->getDefense();
                }
            }
